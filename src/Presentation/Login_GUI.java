...
//		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	...
	private void init() {
		this.loginGuiBll = new Login_GUI_BLL() {
			@Override
			public void notification(String mess) {
				Login_GUI.this.notification(mess);
			}
	...
	private void addAction() {
		login_button.addActionListener(e -> {
			...
			loginGuiBll.onLoginClick(username, password);
		});

		// TODO: register click
		register_button.addActionListener(e -> {
			String password = password_register1.getText();
			String password2 = password_register2.getText();

			if (!password.equals(password2)) {
				notification("Password not match!");
			} else {
				String username = username_register.getText();
				String fullName = name_register.getText();
				int age = Integer.parseInt(age_register.getText());

				loginGuiBll.onRegisterClick(username, password, fullName, age);
			}
		});
	}
